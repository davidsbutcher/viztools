   ...
   function(
      ...
      proteoform_heatmap <-
         ...
         ggplot2::xlim(
            0,
            plyr::round_any(
               max(proteoform_heatmap_data$mass_bin/1000),
               10, f = ceiling)
         ) +
         ggplot2::scale_y_discrete(limits = rev(levels(proteoform_heatmap_data$fraction))) +
         ggplot2::labs(
            x = "Mass Bin (kDa)",
            y = "Fraction"
         ) +
      ...
      if (savePDF == TRUE) {
         ...
         pdf(
            file = glue::glue("{outputDir}/{pdfName}_proteoform_heatmap.pdf"),
            width = 8,
            height = 5,
            bg = "transparent"
         )
